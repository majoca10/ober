...
function DriverPopup({ trip, setDriverStatus,  }:
  ...
  return (
    <ModalOverlay >
      <Modal>
        <Steps current={1}>
          <Steps.Step title="punto de embarque" icon={customIcon()} description={trip.origin.address} />
          <Steps.Step title="punto de destino" icon={customIcon()} description={trip.destination.address} />
        </Steps>
        <Flex>
          <Flex.Item>
            <ExpectationHeader>Tiempo estimado de viaje: {trip.estimatedTime}</ExpectationHeader>
          </Flex.Item>
          <Flex.Item>
            <ExpectationHeader>kilometraje total : {trip.estimatedDistance}</ExpectationHeader>
          </Flex.Item>
        </Flex>
        <WhiteSpace />
      ...
      <Alert>
        <div>El emparejamiento ya se ha completado</div>
        <Icon style={{ marginTop: '20px' }} type={'cross-circle'} size={'lg'}/>
      </Alert>}
      {status === MATCHING_CANCEL &&
      <Alert >
        <div>El pasajero canceló la solicitud</div>
