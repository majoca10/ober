...
function DriverInfoBox() {
  ...
  const handleClickCancel = () => {
    alert('cancelación de llamada', '¿Está seguro de que desea cancelar la llamada del conductor?', [
      { text: 'Cancel' },
  ...
  useEffect(() => {
    if (data && data.cancelTrip.result === 'canceled') {
      ...
      alert('cancelación de llamada', 'Cancelado.', [
  ...
  return (
    <InfoBox>
      ...
      <CarInfo>
        <div>
        Placa <span>{driver.plateNumber}</span>
        </div>
        <div>
        Tipo de vehiculo <span>{driver.carType}</span>
        </div>
      </CarInfo>
      <Buttons>
        <CancelButton onClick={handleClickCancel}>Cancelar Solicitud</CancelButton>
      </Buttons>
    </InfoBox>
  );
}
